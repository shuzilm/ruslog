...
func TestAllHooks(t *testing.T) {
	...
	assert.Equal(ruslog.ErrorLevel, hook.LastEntry().Level)
	assert.Equal("Hello error", hook.LastEntry().Message)
	assert.Equal(1, len(hook.Entries))

	logger.Warn("Hello warning")
	assert.Equal(ruslog.WarnLevel, hook.LastEntry().Level)
...
func TestLoggingWithHooksRace(t *testing.T) {
	...
	assert.Equal(ruslog.InfoLevel, hook.LastEntry().Level)
...
func TestFatalWithAlternateExit(t *testing.T) {
	...
	assert.Equal(ruslog.FatalLevel, hook.LastEntry().Level)
